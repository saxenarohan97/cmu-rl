...
class MPC:
    ...
    def predict_next_state_model(self, states, actions):
        ...
        net_inputs = np.concatenate([states, actions], axis=-1)
        net_outputs_tensor = self.model(net_inputs)
        net_outputs = []
        for net in range(self.num_nets):
            net_outputs.append([net_outputs_tensor[net][0].detach().cpu().numpy(),
                                net_outputs_tensor[net][1].detach().cpu().numpy()])
        net_outputs = np.array(net_outputs)

        which_net = np.random.randint(self.model.num_nets, size=self.popsize * self.num_particles)
        next_states = np.zeros_like(states)

        for net in range(self.model.num_nets):
            indices = np.where(which_net == net)
            means = net_outputs[net][0][indices]
            logvars = net_outputs[net][1][indices]
            stddevs = np.sqrt(np.exp(logvars))
            next_states[indices] = states[indices] + np.random.normal(means, stddevs)
    ...
    def predict_next_state_gt(self, states, actions):
        ...
        next_states = [self.env.get_nxt_state(state, action) for state, action in zip(states, actions)]
        return next_states
