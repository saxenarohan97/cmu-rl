...
def value_function_to_policy(env, gamma, value_function):
    ...
    for state in range(env.nS):
        ...
        for action in range(env.nA):
            ...
            action_values.append(reward + gamma * value_function[nextstate])
...
def policy_iteration_sync(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    while not policy_stable:
        value_func, num_eval_iter = evaluate_policy_sync(env, value_func, 
                                                         gamma, policy, 
                                                         max_iterations, tol)
...
def value_iteration_sync(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    while num_iters < max_iterations:
        ...
        for state in range(env.nS):
            ...
            updated_value_func[state] = np.max(action_values)
    ...
    return policy, value_func, num_iters
...
def value_iteration_async_ordered(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    while num_iters < max_iterations:
        ...
        for state in range(env.nS):
            ...
            next_val = np.max(action_values)
            delta = max(delta, abs(next_val - value_func[state]))
            value_func[state] = next_val
...
def get_goal(env):
    for k1 in env.P.keys():
        for k2 in env.P[k1].keys():
            el = env.P[k1][k2]
            if el[0][2] == 1:
                return el[0][1]

def get_coor(state, env):
    size = env.nS ** 0.5
    
    return state // size, state % size

def manh_dist(state, goal, env):
    state_coor = get_coor(state, env)
    goal_coor = get_coor(goal, env)
    
    return np.abs(state_coor[0] - goal_coor[0]) + np.abs(state_coor[1] - goal_coor[1])

def order_manhattan(env):
    goal = get_goal(env)
    states_dists = [manh_dist(state, goal, env) for state in range(env.nS)]
    return np.argsort(states_dists)
...
def value_iteration_async_custom(env, gamma, max_iterations=int(1e3), tol=1e-3):
    ...
    while num_iters < max_iterations:
        ...
        for state in order_manhattan(env):
            action_values = []
            for action in range(env.nA):
                prob, nextstate, reward, is_terminal = env.P[state][action][0]
                action_values.append(reward + gamma * value_func[nextstate])
